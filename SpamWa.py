...
os.system("clear")
autoketik(f"{ungu}[{biru}Subscribe Ngab{ungu}] {W}OTAK KOSONG")
time.sleep(2)
os.system("xdg-open https://youtube.com/@OTAKNYAILAN9?si=JC0JFozLUYtAQPPN")
autoketik(f"{ungu}[{biru}P{ungu}] {W}Makasih udah subscribe, semoga work")
time.sleep(5)
os.system("xdg-open https://chat.whatsapp.com/BnSHnd0WtWy7Q9JGCqHi6u")
os.system("clear")
autoketik(f"""{merah}>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
{merah}[{biru}×{putih}] {merah}Pembuat {putih}   : Otak Kosong
{putih}[{merah}×{putih}] {merah}You{putih}Tube {putih}  : OTAK KOSONG
{W}[{Y}×{W}] Ip Kamu {putih}  :{Y} {ip}
{W}[{Y}×{W}] Waktu/Jam {putih}:{Y} {localtime}
{putih}<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<""")

nomor = input(f"{W}[{R}• {kuning}•{hijau}•{W}] {biru}Nomor Targetnya Ngab {W}: ")
